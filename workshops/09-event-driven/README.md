...
コマンド（命令）:           イベント（事実の通知）:
  ...
  ・特定の宛先に送る          ・誰が受け取るか送信者は知らない
  ・1対1                     ・1対多
  ・失敗したら送信者が知る     ・送信者は結果を気にしない
  ・命令形で命名              ・過去形で命名
```

> **なぜイベント駆動で疎結合になるのか？**
> 同期通信では「注文サービスが在庫サービスを直接呼ぶ」→ 在庫サービスがダウンすると注文もできなくなる。イベント駆動では「注文サービスが OrderPlaced イベントを発行する」→ 在庫サービスはそのイベントを購読して自律的に処理する。注文サービスは在庫サービスの存在すら知らなくてよい。
...
At-most-once（最大1回）:
  送信 → 受信？ → 確認なし
  ・メッセージが失われる可能性がある
  ・ログ収集など、多少の欠損が許容される場面

At-least-once（最少1回）:
  送信 → 受信 → 確認 → (タイムアウト時は再送)
  ・メッセージが重複する可能性がある ← 冪等性が必要！
  ・ほとんどのビジネスユースケースで推奨

Exactly-once（正確に1回）:
  送信 → 受信 → 確認 → (トランザクション制御)
  ・実現コストが非常に高い
  ・Kafka Transactions 等で部分的に実現可能
```

> **冪等性が最重要**: At-least-once 配信では同じメッセージが2回届く可能性があります。そのため、同じイベントを2回処理しても結果が変わらない（冪等な）ハンドラを書く必要があります。典型的な実装は「処理済みイベントIDを記録し、重複を検出する」方法です。
...
> **補償トランザクションの注意点**:
> - 補償処理自体も失敗する可能性がある → リトライ機構が必要
> - 「元に戻す」のではなく「打ち消し操作を行う」（決済を取り消すのではなく、返金処理を行う）
> - 補償の順番は実行の逆順で行う
> - すべての操作に補償が定義されている必要がある
...
- [ ] 第5回のDDD（特にドメインイベントの概念）を復習済み
- [ ] 第7回のマイクロサービス（サービス間通信、同期 vs 非同期）を復習済み
- [ ] Node.js の `EventEmitter` の基本的な使い方を知っている（知らなければ軽く調べておく）
...
- [ ] 以下のキーワードについて概要を調べておく
  ...
  - Saga パターン
  - 結果整合性（Eventual Consistency）
- [ ] 自分のプロジェクトで「非同期で処理してもよい」機能と「即座に完了すべき」機能を1つずつ挙げておく
  - 例：メール送信は非同期OK、決済結果の確認は同期が必要

#### 事前ミニ課題（任意・20分程度）

ECサイトの「注文確定」後に起きるべき処理を洗い出し、以下のように分類してみてください。

| 処理 | 同期/非同期 | 失敗したらどうなる？ |
|------|-----------|-----------------|
| 例: 在庫引当 | 同期 | 注文自体を失敗にする |
| 例: 確認メール送信 | 非同期 | 後でリトライすればOK |
| ... | ... | ... |

### 運営側

- [ ] 第8回の課題提出状況の確認
- [ ] EventEmitter を使ったプロセス内イベントのデモ環境の準備
- [ ] CQRS の Write/Read 分離を図解するスライドの準備
- [ ] Saga パターン（コレオグラフィ / オーケストレーション）のフロー図の準備
- [ ] 冪等性の具体的な実装パターン（イベントID記録方式）のコード例の準備
- [ ] （可能であれば）Redis Pub/Sub or BullMQ のデモ環境
