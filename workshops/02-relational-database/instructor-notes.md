...
- [ ] スロークエリの調査→インデックス追加→改善確認の一連の流れをデモできるようにする

### 参加者分析・前回との接続
- [ ] 参加者がふだん使っているDBMS（PostgreSQL / MySQL / SQLite等）を事前アンケートで把握し、説明時に配慮する
- [ ] 第1回のSOLID原則との接続（特にDIPとRepository）を意識し、「データアクセスの抽象化」への伏線を張る
- [ ] 第1回の課題提出状況・理解度を前回講師から引き継ぎ、参加者のレベル感を把握する

### 実践準備
- [ ] デモ用DBにサンプルデータ（数万行程度）を投入し、インデックスの効果が目に見える状態にする
- [ ] デッドロック再現用のSQLスクリプトを2つのターミナルで実行できるよう準備する
- [ ] Prisma / TypeORMでの実装例を用意し、「ORMの裏でどんなSQLが発行されているか」を見せられるようにする
...
### 運用・パフォーマンスの観点
- 本番環境でのDB運用は「設計して終わり」ではない。スロークエリモニタリング、定期的なEXPLAIN確認、インデックスの棚卸しなど、運用を通じて設計を改善し続ける
- 「インデックスは貼ればいいわけではない」— 不要なインデックスがINSERT/UPDATE性能を下げ、ストレージを圧迫することも実例で示す
- パーティショニング、レプリケーション（読み取りレプリカ）の概要にも軽く触れると、フェーズ3（分散システム編）への橋渡しになる

### ORM利用時の落とし穴
- ORMを使うと「SQLを書かなくてよい」と思われがちだが、N+1問題や不必要なSELECT *の発生など、ORMが生成するSQLの質を意識することが重要
- Prisma の `include` による eager loading と lazy loading の違いを実例で見せる
- 「ORMに任せきりにせず、重要なクエリはEXPLAINで確認する」習慣を推奨する

### データの長期的な価値
- 「アプリケーションコードは書き直せるが、データは書き直せない」— データモデルの重要性をビジネス観点から伝える
- 正しいデータモデルがあれば、UIやアプリの作り直しにも耐えられる
