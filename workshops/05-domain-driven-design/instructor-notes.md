...
- [ ] 「DDDを導入しなくてもよいケース」の判断基準を明確に説明できるようにする

### 前回までの接続
- [ ] 第1回（DIP）、第3回（データモデリング）、第4回（Repository, Factory）との接続を整理する
- [ ] 第3回で扱った「ビジネスの構造をモデルで表現する」がDDDの戦術的設計でどう発展するかを整理する
- [ ] 第4回のRepositoryパターンが集約の永続化でどう使われるかを具体的に示せるようにする

### ワークショップの準備
- [ ] ユビキタス言語の策定ワーク用に、参加者が取り組みやすい業務ドメイン（ECサイト、予約システム等）の要件概要を準備する
- [ ] 集約の境界を議論するためのケーススタディ（「Order集約にCustomerを含めるべきか」等）を2〜3パターン用意する
- [ ] 「イベントストーミング」の手法を簡易的に紹介できるスライドを1〜2枚用意する（DDDの新しいモデリング手法として）
...
### ビジネスサイドとの協働
- DDDの最大の価値は技術的な実装パターンではなく、ビジネスサイドと技術サイドが「同じ言葉」で会話できるようになること
- 「ユビキタス言語をコードに反映する」ことで、ビジネスの人がコードを読めなくても、メソッド名やクラス名から設計意図が伝わる
- 実際のプロジェクトでは、ドメインエキスパートとの定期的な対話がDDDの成否を分ける

### 既存プロジェクトへの段階的導入
- 「全面的にDDDで書き直す」のは現実的でない場合が多い。以下の段階的アプローチを紹介する:
  1. まずユビキタス言語の整理から始める（コストゼロ）
  2. 新規機能から値オブジェクトを導入する
  3. ビジネスロジックが複雑な箇所にエンティティ・集約を適用する
  4. 必要に応じて境界づけられたコンテキストを明確にする
- 「DDDのすべてを一度に導入する必要はない」というメッセージが、参加者のプレッシャーを軽減する

### データモデルとドメインモデルの関係
- 第3回で学んだデータモデルとDDDのドメインモデルは別物であることを明確にする
- テーブル構造とドメインオブジェクトが1対1対応しないケース（集約を複数テーブルにマッピングする等）を具体例で示す
- 「データ中心設計」と「ドメイン中心設計」の違いを対比する
