...
- [ ] 「パターンを使わないシンプルなコード」と「パターンを適用したコード」の行数・複雑さの比較を準備する

### 参加者の実務との接続
- [ ] 参加者が使っているフレームワークに組み込まれているパターンを調査し、「実は既に使っている」ことを示す準備をする
- [ ] 参加者のプロジェクトで「パターンを適用すべき場面」の具体例を1つ考えておく（事前アンケートの技術スタックに基づく）

### 後続回への布石
- [ ] Repositoryパターンが第5回（DDD）・第6回（クリーンアーキテクチャ）でどう発展するかを整理する
- [ ] Observerパターンが第9回（イベント駆動）の基礎概念であることを意識して説明する準備をする
...
### テスタビリティの観点
- パターンを適用する最大の実利の1つは「テストが書きやすくなる」こと。Repositoryパターンによるモック差し替え、Strategyパターンによる振る舞いの差し替えなど、テストとの関連を意識して説明する
- InMemoryRepository を使ったユニットテストのデモは、参加者に「これは便利」と実感してもらいやすいポイント

### リファクタリングの観点
- パターンは「最初から適用する」のではなく「コードの問題が見えたときにリファクタリングで適用する」のが健全なアプローチ
- 「コードの臭い（Code Smell）→ パターンの適用」という流れを強調する
- 3回同じ構造が出てきたらパターン化を検討する（Rule of Three）

### モダンフレームワークの観点
- React: カスタムHooks ≒ Strategy、Context API ≒ DI/Service Locator
- NestJS: Module ≒ Facade、Provider ≒ Factory、Interceptor ≒ Chain of Responsibility
- 「フレームワークがパターンを内包している」ことを知ると、フレームワークの設計意図が理解できるようになる
